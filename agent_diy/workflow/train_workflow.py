...
def _initialize_training_state(env, agent, logger):
    ...
    # Create the rollout storage and set it on the algorithm for use in learn()
    # 创建 rollout 存储缓冲区并设置到 algorithm 上供 learn() 使用
    storage = RolloutStorage(
        num_envs=agent.num_envs,
        num_transitions_per_env=stage.num_steps_per_env,
        obs_shape=(obs.shape[1],),
        privileged_obs_shape=(critic_obs.shape[1],),
        actions_shape=(stage.num_actions,),
        device=agent.device,
    )
    agent.algorithm.storage = storage
...
def _update_transition_data(
    ...
    if time_outs is not None:
        if isinstance(time_outs, torch.Tensor):
            time_outs_t = time_outs.to(agent.device)
            transition.rewards += agent.algorithm.gamma * torch.squeeze(
                transition.values * time_outs_t.unsqueeze(1), 1
            )
...
def _compute_advantages_and_returns(storage, agent, critic_obs, logger):
    ...
    last_values = agent.model.evaluate(last_critic_obs.detach()).detach()
